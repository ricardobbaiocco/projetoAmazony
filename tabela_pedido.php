...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top">
    ...
    <div class="container formularios">
        ...
        <h2>Tabela de pedidos de produtos</h2>
        <br>
        <div class="table-responsive">
            <table class="table table-striped">
                ...
                <tbody>
                    ...
                    $query = "SELECT pedido.idPedido, pedido.dataCriacao, pedido.quantItem, pedido.valorTotalPedido, pessoa.nome ,pessoa.cpf FROM pedido
                    INNER JOIN pessoa  ON pessoa.idPessoa = pedido.fk_pessoa_idPessoa ORDER BY dataCriacao";
                    ...
                    if ($resultado) {
                        ...
                        while ($row = sqlsrv_fetch_array($resultado, SQLSRV_FETCH_ASSOC)) {
                            ...
                            echo '<td>' . $row['dataCriacao']->format('d/m/Y') . '</td>';
                            echo '<td>' . $row['quantItem'] . '</td>';
                            echo '<td>R$ ' . number_format($row['valorTotalPedido'], 2, ',', '.') . '</td>';
                            echo '<td>' . $row['nome'] . '</td>';
                            echo '<td>' . $row['cpf'] . '</td>';
                            echo '</td>';
                            echo '</tr>';
